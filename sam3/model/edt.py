...
# Try to import triton, but provide fallback if not available (e.g., on macOS)
try:
    import triton
...
def _edt_opencv_fallback(data: torch.Tensor) -> torch.Tensor:
    """
    OpenCV-based fallback for EDT when Triton is not available.
    ...
    for i in range(B):
        # cv2.distanceTransform expects uint8 input where 0 is background
        mask = data_np[i].astype(np.uint8)
        output[i] = cv2.distanceTransform(mask, cv2.DIST_L2, 0)
...
if TRITON_AVAILABLE:
    ...
    def _edt_triton_impl(data: torch.Tensor):
        """
        Triton-based EDT implementation for CUDA.
        """
        assert data.dim() == 3
        assert data.is_cuda
        B, H, W = data.shape
